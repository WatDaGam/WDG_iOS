...
class AuthModel: ObservableObject {
    ...
    func handleAppleLogin(userId: String) {
        Task {
            isLoggedIn = await loginWithApple(userId: userId)
            if !isLoggedIn { loginFailedAlert = true }
        }
    }
    func loginWithApple(userId: String) async -> Bool {
        await withCheckedContinuation { continuation in
            let serverURLString = Bundle.main.infoDictionary?["SERVER_URL"] as? String ?? ""
            guard let loginURL = URL(string: "https://\(serverURLString)/login?platform=APPLE") else {
                print("Invalid URL")
                return
            }
